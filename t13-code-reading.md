...
`git grep`は、バージョン管理対象となっているカレントディレクトリ以下のすべてのファイルを対象に検索してくれるので、わざわざファイル指定しなくてよい分、効率がよい。
...
ただし、ドキュメントとソースコードの間に乖離がある可能性は一応頭の片隅に入れておく。

### ソースコードとコメントから自動的に生成できるAPIドキュメントを読む

JavaならJavaDoc、PythonならPyDoc、RubyならRDocなど、
